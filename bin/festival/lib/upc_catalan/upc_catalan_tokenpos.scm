...
(if (and (string-matches (string-append (car (symbolexplode name))) "[A-ZÀÈÉÍÒÓÚ]") (string-matches (string-append (car (cdr (symbolexplode name)))) "[a-zàéèíòóú]"))
   "1"
   "0"))
...
Returns 1 if sc's name have two capitals letters (one at start) with a no capital letter between them at least, 0 otherwise"
  (if (string-matches name "[A-Z][a-zàèéíòóúüïç]+[A-ZÀÈÉÍÒÓUÏÜ].*")
      "1"
      "0"))
